package pasa.darrylburke.renderer;
...
public class IconTableCellRenderer extends DefaultTableCellRenderer {
   ...
   public Component getTableCellRendererComponent(final JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
      ...
      setIcon(null);
      setText("width=" + icon.getIconWidth() + ", height=" + icon.getIconHeight());
      ...
      if (getIcon() == null) {
         try {
            createIcon(defaultIcon == null ? new XIcon(icon.getIconWidth(), icon.getIconHeight()) : defaultIcon, null);
         } catch (Exception ex) {
         }
      }
      setBackground(getCorrectBackground(table, isSelected));
   ...
   public static void setRowHeightsForIcons(JTable table) {
      ...
      for (int col = 0; col < table.getColumnCount(); col++) {
         if (Icon.class.isAssignableFrom(table.getColumnClass(col)) && table.getDefaultRenderer(Icon.class) instanceof IconTableCellRenderer) {
